...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    result = []
    for each_id in ids:
        result.append(id_to_token[each_id])
    return result
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    if len(ids) < max_len: 
        ...
        temp = [ids[i] for i in range(max_len)]
        ids = temp
    return ids
...
def compute_positional_div_term(d_model):
    # TODO: return a 1D FloatTensor of length d_model // 2 holding the sinusoidal frequency divisors
     return torch.tensor(
        [10000 ** (-2 * i / d_model) for i in range(d_model // 2)],
        dtype=torch.float
    )
...
def build_position_index_column(max_len):
    ...
    return torch.arange(max_len, dtype=torch.float).reshape(max_len, 1)
...
def softmax_attention_weights(masked_scores):
    ...
    # If an entire row was -inf, softmax produces NaN.
    # Replace those NaNs with 0.
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    # 1. Raw scores: Q @ K^T
    scores = query @ key.transpose(-2, -1) # (B, H, Lq, Lk)
    
    # 2. Scale by sqrt(d_k)
    scores = scores / math.sqrt(d_k) # (B, H, Lq, Lk)

    # 3. Apply mask if provided
    if mask is not None:
        scores = scores.masked_fill(~mask, float("-inf")) # (B, H, Lq, Lk)

    # 4. Softmax over keys
    attention_weights = torch.softmax(scores, dim=-1) # (B, H, Lq, Lk)
    attention_weights = torch.nan_to_num(attention_weights, nan=0.0) # (B, H, Lq, Lk)

    # 5. Weighted sum of values
    context = attention_weights @ value  # (B, H, Lq, d_v)
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    B, head_num, L, d_k = multi_head_tensor.shape
    multi_head_tensor = torch.transpose(multi_head_tensor, 1, 2)
    return multi_head_tensor.reshape(B, L, head_num * d_k)
...
def apply_linear_projection(x, weight, bias):
    ...
    if bias == None:
        bias = 0
    return x @ torch.transpose(weight, 0, 1) + bias
...
def multi_head_scaled_dot_product_attention(q_h, k_h, v_h, mask=None):
    ...
    d_k = k_h.shape[-1]
    scores = q_h @ torch.transpose(k_h, -2, -1)
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    result = merge_heads_back_to_model_dim(context)
...
def apply_residual_add_and_norm(residual_input, sublayer_output, gamma, beta, eps=1e-5):
    ...
    out = residual_input + sublayer_output
    return normalize_and_scale_with_gamma_beta(out, gamma, beta, eps)
...
def apply_dropout_with_keep_mask(x, keep_mask, keep_prob):
    ...
    return x.masked_fill(~keep_mask, 0.0) / (keep_prob)
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    sublay_output = assemble_multi_head_attention_forward(x, x, x, w_q, w_k, w_v, w_o, num_heads, src_mask)
    result = apply_residual_add_and_norm(x, sublay_output, gamma, beta)
    return result
...
def encoder_layer_feed_forward_sublayer(x, w1, b1, w2, b2, gamma, beta):
    ...
    result = apply_residual_add_and_norm(x, sublayer_output, gamma, beta)
    return result
...
def assemble_encoder_layer(x, layer_params, num_heads, src_mask):
    ...
    attn_layer_output = encoder_layer_self_attention_sublayer(x, layer_params['w_q'], layer_params['w_k'], layer_params['w_v'], layer_params['w_o'], layer_params['attn_gamma'], layer_params['attn_beta'], num_heads, src_mask)
    ffn_layer_output = encoder_layer_feed_forward_sublayer(attn_layer_output, layer_params['w1'], layer_params['b1'], layer_params['w2'], layer_params['b2'], layer_params['ffn_gamma'], layer_params['ffn_beta'])
...
def decoder_layer_masked_self_attention_sublayer(y, w_q, w_k, w_v, w_o, gamma, beta, num_heads, tgt_mask):
    ...
    attn = assemble_multi_head_attention_forward(y, y, y, w_q, w_k, w_v, w_o, num_heads, tgt_mask)
    result = apply_residual_add_and_norm(y, attn, gamma, beta)
    return result
...
def decoder_layer_cross_attention_sublayer(y, encoder_output, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    cross_attn = assemble_multi_head_attention_forward(y, encoder_output, encoder_output, w_q, w_k, w_v, w_o, num_heads, src_mask)
    result = apply_residual_add_and_norm(y, cross_attn, gamma, beta)
    return result
...
def decoder_layer_feed_forward_sublayer(y, w1, b1, w2, b2, gamma, beta):
    ...
    sublayer_output = position_wise_feed_forward_network(y, w1, b1, w2, b2)
    result = apply_residual_add_and_norm(y, sublayer_output, gamma, beta)
    return result
...
def assemble_decoder_layer(y, encoder_output, layer_params, num_heads, src_mask, tgt_mask):
    ...
    self_attn_output = decoder_layer_masked_self_attention_sublayer(
        y,
    ...
    # 3. Feed-forward
    ffn_output = decoder_layer_feed_forward_sublayer(
        cross_attn_output,
        layer_params['w1'],
        layer_params['b1'],
        layer_params['w2'],
...
def stack_decoder_layers(y, encoder_output, decoder_layer_params_list, num_heads, src_mask, tgt_mask):
    ...
    for p in decoder_layer_params_list:
        y = assemble_decoder_layer(y, encoder_output, p, num_heads, src_mask, tgt_mask)
    return y
...
def apply_final_output_projection(decoder_output, output_projection_weight, output_projection_bias=None):
    ...
    if output_projection_bias == None:
        output_projection_bias = 0
    return decoder_output @ output_projection_weight.transpose(-1, -2) + output_projection_bias
...
def run_transformer_forward(src_ids, tgt_ids, model_params, num_heads, pad_id):
    ...
    # shared embedding table, scaled, plus positional encoding
    src = emb[src_ids] * math.sqrt(d_model)
    tgt = emb[tgt_ids] * math.sqrt(d_model)
    src = src + build_sinusoidal_positional_encoding(S, d_model)
    tgt = tgt + build_sinusoidal_positional_encoding(T, d_model)

    # source padding mask -> (B, 1, 1, S)
    src_mask = (src_ids != pad_id)[:, None, None, :]

    # target: padding AND causal -> (B, 1, T, T)
    tgt_pad = (tgt_ids != pad_id)[:, None, None, :]
    ...
    for layer in model_params["encoder_layers"]:
        memory = assemble_encoder_layer(memory, layer, num_heads, src_mask)

    y = tgt
    for layer in model_params["decoder_layers"]:
        y = assemble_decoder_layer(y, memory, layer, num_heads, src_mask, tgt_mask)

    logits = y @ model_params["output_projection"].T
    return torch.log_softmax(logits, dim=-1)
...
def init_encoder_layer_parameters(d_model, num_heads, d_ff):
    ...
    return encoder_layer_parameters
...
def init_decoder_layer_parameters(d_model, num_heads, d_ff):
    ...
    return decoder_layer_parameters
...
def init_embedding_and_projection_parameters(vocab_size, d_model, tie_weights=True):
    ...
    embedding_parameters={
        'src_embedding': xavier((vocab_size, d_model)),
        'tgt_embedding': xavier((vocab_size, d_model))
    }

    if tie_weights:
        embedding_parameters['output_projection'] = embedding_parameters['tgt_embedding']
    else:
         embedding_parameters['output_projection'] = xavier((vocab_size, d_model))
...
def compute_noam_learning_rate(step, d_model, warmup_steps):
    ...
    # lr = d_model^(-1/2) * min(step^(-1/2), step * warmup_steps ^ (-1.5))
...
def build_uniform_smoothing_distribution(shape, vocab_size, epsilon):
    ...
    return torch.full(shape, value, dtype=torch.float)
...
def zero_pad_column_and_pad_token_rows(smoothed_distribution, gold_token_ids, pad_id):
    ...
    # 1. zero the PAD vocabulary column
    result[..., pad_id] = 0.0

    # 2. zero whole rows whose gold token is PAD
